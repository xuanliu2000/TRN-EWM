...
def conv1x1(in_planes, out_planes, stride=1):
    """1x1 convolution"""
    return MetaConv1d(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)

def conv3x1(in_planes, out_planes, stride=1):
    """3x3 convolution with padding"""
    return MetaConv1d(in_planes, out_planes, kernel_size=3, stride=stride,
    ...
    def update_params(self, lr_inner, first_order=False, source_params=None, detach=False):
        if source_params is not None:
            for tgt, src in zip(self.named_params(self), source_params):
                ...
                #param_t = Variable(param_t)
                #print('paramt',param_t.size())
                tmp =(param_t) - (lr_inner * grad)
                #tmp = param_t.add_(-lr_inner * grad)
...
class MetaConv1d(MetaModule):   #meta conv1d
    def __init__(self, *args, **kwargs):
        super().__init__()
...
class MetaConvTranspose1d(MetaModule):
    def __init__(self, *args, **kwargs):
        super().__init__()
        ignore = nn.ConvTranspose1d(*args, **kwargs)
...
def _weights_init(m):
    classname = m.__class__.__name__
    # print(classname)
    if isinstance(m, MetaLinear) or isinstance(m, MetaConv1d):
        init.kaiming_normal_(m.weight)

def _weights_init3(m):
        if isinstance(m, MetaConv1d):
...
class Resnet18(MetaModule):
    def __init__(self, block=BasicBlock2, num_blocks=[2, 2, 2, 2],in_channel=1, out_channel=64,
                                    zero_init_residual=False):
        super(Resnet18, self).__init__()

        self.in_planes = 64
        self.conv1 = MetaConv1d(in_channel, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        ...
        self.linear = MetaLinear(512 * block.expansion, out_channel)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.final_feat_dim = out_channel

        self.apply(_weights_init3)
    ...
    def _make_layer(self, block, planes, blocks, stride):
        ...
        if stride != 1 or self.in_planes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                MetaBatchNorm1d(planes * block.expansion),
            )
        #print(downsample)
        layers = []
